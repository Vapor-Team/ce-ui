...
const buildPackagesEntry = () => {
  ...
  const componentsIndexContent = `${TIPS}\nimport { CreateComponents } from '@lib/ts-utils/create-basic'\n${handlerArr(
    ...
    return `export const ${humpConversion(
      name
    )} = CreateComponents(${humpConversion(name)}_)`
  }).join('\n')}\nexport default {\n  ${Components_.map((name) => {
  ...
  const content = `${TIPS}\nimport { ${Components_.map((name) => {
    ...
    ', '
  )} } from './directives'\nconst version = '${version}'\nconst components = [ ${Components_.map(
    (name) => {
      return humpConversion(name)
    }
  ...
  fs.writeFileSync(
    path.join(__dirname, '../packages/components/index.ts'),
